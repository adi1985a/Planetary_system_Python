...
*   💥 **Planet Collisions**: Models and visualizes collisions between planets or other celestial bodies, including spectacular collision animations.
*   ⏳ **Time Dilation Effects**: Incorporates a representation of time dilation, influenced by proximity to strong gravitational fields (like black holes) or high velocities.
...
*   🌀 **Realistic Slingshot & Ejection**: Planets can be ejected from their orbits by close encounters with black holes, following realistic gravitational physics.
*   🌠 **Animated Visuals**: Includes animated backgrounds, glowing effects, and dynamic orbit deformation.
...
*   **Speed Controls**: UI buttons to adjust simulation speed (0.5x, 1x, 2x, 5x).
*   **Black Hole Size Adjustment**: UI buttons to control the size/mass of newly created or selected black holes.
*   **Reset Simulation**: A dedicated button to revert the simulation to its default starting conditions.
*   **Save System State**: A button to save the current state of all objects in the simulation.
*   **Load System State**: A button to load a previously saved simulation state.
...
*   **User Interface**: The UI for controls (speed, black hole size, save/load) is implemented using Pygame's drawing and event handling capabilities (e.g., clickable areas, text rendering).
*   **Save/Load Format**: The format for saving and loading simulation states is JSON.
